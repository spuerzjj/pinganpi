...
**当前开发基线：** 已完成阶段 10 AI 代笔接入设计；阶段 11 的本地最小 AI 代理脚手架已完成，真实 MiMo env 连通验证和云端落点配置仍待确认。精确提交以 `git log --oneline --decorate -5` 为准。
...
主线阶段按“本地体验稳定 → AI 接入设计 → 最小 AI / 云配置 → AI 正文生成 → 同步模型 → 双人真实可用 → 异常与附件 → 发布质量”推进。当前完成 10 / 18 个阶段，阶段 11 正在推进。
...
| 11 | 云服务器与 AI 能力最小配置 | 进行中 | 本地最小 AI 代理脚手架已完成；真实 MiMo env 连通、费用告警和云端落点仍待。 |
...
- AI 代笔生成信件正文：写信页尚未接入 AI adapter；后续需要保留手工校改，失败时保存口述草稿等待稍后起稿。
- 云端同步、双人账户绑定和真实双设备数据同步。
- 阶段 11 的云端落点、真实 MiMo 环境变量连通验证、AI 能力额度、费用限额、权限和部署凭据配置。
...
状态：进行中。本地最小 AI 代理脚手架已完成；真实 MiMo 连接验证、费用告警和云端落点配置仍待。
...
- 用户 Xiaomi MiMo 订阅页显示的 OpenAI-compatible `MIMO_API_BASE_URL`。
- 用户 Xiaomi MiMo 订阅页显示的 `MIMO_MODEL_ID`。
- 用户 key 类型是 Token Plan `tp-` 还是按量 `sk-`，以及对应额度、过期和费用提醒方式。
- 腾讯云 CloudBase 是否作为阶段 11 的实际部署落点；当前仓库只完成本地可测代理，不等于已配置云端资源。

目标：在 AI 接入设计明确后，基于已购买的 Xiaomi MiMo 模型创建最小可用 AI 服务端代理环境，并把费用、权限和配置固化为可复现的开发基础设施。

推荐范围：

- 选择云服务落点，优先评估腾讯云 CloudBase 是否适合承载最小 AI 服务端代理。
- 默认优先接入用户已购买的 Xiaomi MiMo 模型；阶段 11 实施时确认 API base URL、模型 ID、调用协议、额度上限、费用告警和响应质量。
