...
**当前开发基线：** 已完成阶段 11 本地 AI 代理基础；真实 MiMo env、费用告警和云端落点配置已拆为独立阶段 13。精确提交以 `git log --oneline --decorate -5` 为准。
...
主线阶段按“本地体验稳定 → AI 接入设计 → 本地 AI 代理 → AI 正文生成 → 云端 AI 配置 → 同步模型 → 双人真实可用 → 异常与附件 → 发布质量”推进。当前完成 11 / 19 个阶段，阶段 12 正在推进。
...
| 13 | 云端 AI 代理与费用配置 | 未开始 | 真实 MiMo env、云函数 / CloudBase 落点、费用告警、secret 和回滚删除步骤。 |
...
- 阶段 11 已提供阶段 12 开发期所需的本地代理边界；阶段 12 可先调用本地代理或 fake adapter，不阻塞于云端购买和部署。
