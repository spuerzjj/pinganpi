...
> 本设计对应路线图阶段 10：AI 代笔接入设计。目标是为后续阶段 11 的最小 AI 服务端代理配置、阶段 12 的 AI 正文生成落地提供明确边界。
...
- 阶段 11 需要在实施时确认 Xiaomi MiMo 的接口地址、模型 ID、调用协议和额度告警方式。
- 阶段 12 需要根据供应商返回格式确定服务端代理的响应清洗规则。
- 如果用户后续希望完全离线可起稿，需要另行设计本地模型或重新引入非 AI 草稿生成机制；当前不做。

## 阶段 11 当前供应商记录

用户已购买 Xiaomi MiMo 模型，阶段 11 默认优先接入该模型，不再从零采购新的 AI 能力。实施前仍需确认：
