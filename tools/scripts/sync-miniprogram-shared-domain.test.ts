...
async function createFixture() {
  ...
  const targetDir = join(root, "apps", "miniprogram", "shared", "domain");
...
describe("sync miniprogram shared domain", () => {
  ...
  it("reports missing and changed copies in check mode without writing files", async () => {
    ...
    expect(result.errors).toEqual([
      "缺少副本：apps/miniprogram/shared/domain/money.ts",
      "内容不一致：apps/miniprogram/shared/domain/time.ts",
    ]);
  ...
  it("reports extra TypeScript files in the target directory", async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors).toEqual(["多余文件：apps/miniprogram/shared/domain/extra.ts"]);
  });
  ...
  it("discovers new source TypeScript files and reports missing target copies", async () => {
    ...
    expect(result.ok).toBe(false);
    expect(result.errors).toEqual(["缺少副本：apps/miniprogram/shared/domain/new-helper.ts"]);
  });
});
